from CruellaRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from CruellaRobot import DRAGONS
...
async def kaizuryu_is_admin(chat_id: int):
    status = False
    kaizuryu = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if kaizuryu.id == user.id:
            status = True
            break
    return status
